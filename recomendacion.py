...
def recomendar_cancion(categoria, preferencias, ruta="data_moods.csv"):
    ...
    if canciones_categoria.empty:
        print(f"  No se encontraron canciones para '{categoria}'.")
        return None
    ...
    if preferencias:
        ...
        pool = canciones_preferidas if not canciones_preferidas.empty else canciones_categoria
    else:
        pool = canciones_categoria

    cancion = pool.sample(n=1).iloc[0]
    return {
        "nombre":  cancion["name"],
